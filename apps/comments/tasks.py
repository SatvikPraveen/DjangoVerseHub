...
def _wants_email(user):
    ...
    if profile is not None and not getattr(profile, 'email_notifications', True):
        return False
    return True
...
def send_comment_like_notification(self, comment_id, liker_id):
    ...
    context = build_comment_email_context(
        comment, recipient, liker=liker, liker_name=_display_name(liker)
    )
...
def cleanup_old_flagged_comments(days=30):
    ...
    count, _ = Comment.objects.filter(
        is_flagged=True, is_active=False, updated_at__lt=cutoff
    ).delete()
