...
class CommentAPITest(TestCase):
    def setUp(self):
        ...
        self.comment = Comment.objects.create(
            author=self.user, content="Test comment content", content_object=self.article
        )
    ...
    def test_create_comment_invalid_content_type(self):
        ...
        data = {"content": "Valid comment content", "content_type": "invalid.model", "object_id": str(self.article.id)}
        response = self.client.post(url, data)
    ...
    def test_like_comment_action(self):
        ...
        self.assertTrue(CommentLike.objects.filter(comment=self.comment, user=self.other_user).exists())

    def test_unlike_comment_action(self):
        # Create existing like
        CommentLike.objects.create(comment=self.comment, user=self.other_user)
    ...
    def test_tree_action(self):
        ...
        Comment.objects.create(
            author=self.other_user, content="Reply comment", content_object=self.article, parent=self.comment
        )

        url = reverse("comments:comment-tree")
        content_type = ContentType.objects.get_for_model(self.article)
        response = self.client.get(
            url, {"content_type": f"{content_type.app_label}.{content_type.model}", "object_id": str(self.article.id)}
        )
