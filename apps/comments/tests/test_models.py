...
class CommentModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email="test@example.com", first_name="Test", last_name="User")
        self.other_user = User.objects.create_user(email="other@example.com", first_name="Other", last_name="User")

        # Create an article to comment on
        self.article = Article.objects.create(
            title="Test Article", content="Test content" * 20, author=self.user, status="published"
        )

        self.comment = Comment.objects.create(
            author=self.user, content="This is a test comment.", content_object=self.article
        )
    ...
    def test_is_root_property(self):
        ...
        reply = Comment.objects.create(
            author=self.other_user, content="This is a reply.", content_object=self.article, parent=self.comment
        )
    ...
    def test_get_thread_depth(self):
        ...
        reply1 = Comment.objects.create(
            author=self.other_user, content="Reply level 1", content_object=self.article, parent=self.comment
        )
        self.assertEqual(reply1.get_thread_depth(), 1)

        reply2 = Comment.objects.create(
            author=self.user, content="Reply level 2", content_object=self.article, parent=reply1
        )
    ...
    def test_total_replies_property(self):
        ...
        # Add nested replies
        reply1 = Comment.objects.create(
            author=self.other_user, content="Reply 1", content_object=self.article, parent=self.comment
        )

        Comment.objects.create(author=self.user, content="Reply 2", content_object=self.article, parent=reply1)
    ...
    def test_validation_parent_same_object(self):
        ...
        other_article = Article.objects.create(
            title="Other Article", content="Other content" * 20, author=self.user, status="published"
        )

        other_comment = Comment.objects.create(
            author=self.user, content="Comment on other article", content_object=other_article
        )
    ...
    def test_validation_thread_depth(self):
        ...
        reply2 = Comment.objects.create(author=self.user, content="Reply 2", content_object=self.article, parent=reply1)

        reply3 = Comment.objects.create(
            author=self.other_user, content="Reply 3", content_object=self.article, parent=reply2
        )

        # Try to create 4th level reply (should fail)
        with self.assertRaises(ValidationError):
            invalid_reply = Comment(
                author=self.user, content="Reply 4 - too deep", content_object=self.article, parent=reply3
            )
            invalid_reply.clean()
...
class CommentManagerTest(TestCase):
    def setUp(self):
        ...
        self.active_comment = Comment.objects.create(
            author=self.user, content="Active comment", content_object=self.article, is_active=True
        )

        # Create inactive comment
        self.inactive_comment = Comment.objects.create(
            author=self.user, content="Inactive comment", content_object=self.article, is_active=False
        )
    ...
    def test_root_comments_manager(self):
        # Create a reply
        reply = Comment.objects.create(
            author=self.user, content="Reply comment", content_object=self.article, parent=self.active_comment
        )
    ...
    def test_thread_comments_manager(self):
        # Create replies
        reply1 = Comment.objects.create(
            author=self.user, content="Reply 1", content_object=self.article, parent=self.active_comment
        )

        reply2 = Comment.objects.create(
            author=self.user, content="Reply 2", content_object=self.article, parent=self.active_comment
        )
...
class CommentLikeModelTest(TestCase):
    def setUp(self):
        ...
        self.comment = Comment.objects.create(author=self.user, content="Test comment", content_object=self.article)
