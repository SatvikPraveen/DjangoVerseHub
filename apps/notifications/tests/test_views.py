...
class NotificationJSONAPITest(NotificationViewTestBase):
    ...
    def test_cannot_mark_or_delete_others_notifications(self):
        ...
        self.assertEqual(
            self.api.post(reverse("notifications:api_mark_read", kwargs={"notification_id": n.pk})).status_code, 404
        )
        self.assertEqual(
            self.api.delete(reverse("notifications:api_delete", kwargs={"notification_id": n.pk})).status_code, 404
        )
        n.refresh_from_db()
...
class NotificationViewSetTest(NotificationViewTestBase):
    ...
    def test_actions(self):
        ...
        response = self.api.patch(
            reverse("api:notification-preferences"), {"digest_frequency": "weekly"}, format="json"
        )
        self.assertEqual(response.status_code, 200)
