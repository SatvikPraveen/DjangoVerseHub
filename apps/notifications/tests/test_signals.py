...
class SignalTestBase(TestCase):
    ...
    def setUp(self):
        ...
        for target in ('apps.comments.signals.send_comment_notification',
                       'apps.comments.signals.moderate_comment'):
    ...
    def make_comment(self, author, content='Nice!', parent=None):
        return Comment.objects.create(
            author=author, content_type=self.article_ct,
            object_id=str(self.article.pk), content=content, parent=parent,
        )
...
class CommentNotificationSignalTest(SignalTestBase):
    ...
    def test_duplicate_unread_notification_is_not_created(self, mock_send):
        ...
        self.assertEqual(
            Notification.objects.filter(recipient=self.author, sender=self.commenter, notification_type='comment').count(),
            1,
        )
        self.assertEqual(mock_send.call_count, 1)
...
class ArticlePublishedSignalTest(TestCase):
    ...
    def _article(self, status, **kwargs):
        return Article.objects.create(
            title=kwargs.pop('title', 'Hello World'), author=self.author,
            category=self.category, content='body', status=status, **kwargs,
        )
    ...
    def test_published_article_notifies_followers(self, mock_send):
        ...
        notifications = Notification.objects.filter(notification_type='post', sender=self.author).order_by('recipient__username')
...
class SendNotificationToUserTest(TestCase):
    ...
    def setUp(self):
        ...
        self.notification = Notification.objects.create(
            recipient=self.user1, sender=self.user2, notification_type='comment', message='Test notification',
        )
    ...
    def test_immediate_email_queued_when_digest_is_none(self):
        ...
        NotificationPreference.objects.create(
            user=self.user1, digest_frequency='none', email_comment=True,
        )
        mail.outbox.clear()
