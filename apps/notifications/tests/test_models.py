...
class NotificationModelTest(TestCase):
    def setUp(self):
        self.user1 = User.objects.create_user(email="user1@test.com", password="testpass123", username="user1")
        self.user2 = User.objects.create_user(email="user2@test.com", password="testpass123", username="user2")
...
class NotificationManagerTest(TestCase):
    ...
    def test_notify_bulk_creates_and_skips_sender(self):
        created = Notification.objects.notify(
            [self.user1, self.user2, self.user3, self.user1],
            self.user1,
            "post",
            "New post",
            target=self.article,
    ...
    def test_notify_dedupes_unread_only(self):
        ...
        self.assertEqual(
            len(Notification.objects.notify([self.user2], self.user1, "like", "liked", target=self.article)), 1
        )
        self.assertEqual(
            len(
                Notification.objects.notify(
                    [self.user2], self.user1, "like", "again", target=self.article, dedupe=False
                )
            ),
            1,
        )
    ...
    def test_notify_respects_in_app_preference(self):
        ...
        self.assertEqual(
            Notification.objects.notify([self.user2, self.user3], self.user1, "like", "liked"),
            [self.user3.notifications.get()],
        )
...
class NotificationPreferenceModelTest(TestCase):
    ...
    def test_allows_in_app_and_email(self):
        prefs = NotificationPreference.objects.create(
            user=self.user, in_app_like=False, email_like=True, email_post=False
        )
