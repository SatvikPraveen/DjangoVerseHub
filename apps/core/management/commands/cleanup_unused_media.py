...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--dry-run", action="store_true", help="Show what would be deleted without actually deleting files"
        )
        parser.add_argument(
            "--older-than", type=int, default=30, help="Only delete files older than N days (default: 30)"
        )
        parser.add_argument(
            "--file-types",
            nargs="+",
        ...
        parser.add_argument(
            "--exclude-dirs", nargs="+", default=["admin", "cache"], help="Directories to exclude from cleanup"
        )
        parser.add_argument("--min-size", type=int, help="Only delete files larger than N bytes")
        parser.add_argument("--interactive", action="store_true", help="Ask for confirmation before deleting each file")
        parser.add_argument("--stats-only", action="store_true", help="Only show statistics without cleaning up")
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Starting media files cleanup..."))

        if self.dry_run:
            self.stdout.write(self.style.WARNING("DRY RUN MODE: No files will be actually deleted"))
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS("Media cleanup completed successfully!"))
    ...
    def get_referenced_files(self):
        ...
        self.stdout.write(self.style.SUCCESS(f"Found {len(referenced_files)} referenced files"))
    ...
    def get_all_media_files(self):
        ...
        self.stdout.write(self.style.SUCCESS(f"Found {len(media_files)} files in media directory"))
    ...
    def find_unused_files(self, referenced_files, all_media_files):
        ...
        self.stdout.write(self.style.WARNING(f"Found {len(unused_files)} unused files"))
    ...
    def show_statistics(self, all_files, referenced_files, unused_files, filtered_files):
        ...
        if file_types:
            ...
            for ext, info in sorted(file_types.items()):
                self.stdout.write(f"{ext}: {info['count']} files, {self.format_size(info['size'])}")
    ...
    def cleanup_empty_directories(self):
        ...
        for root, dirs, _files in os.walk(media_root, topdown=False):
