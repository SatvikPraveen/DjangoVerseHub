...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--group-name", help="Group name when using --recipients=group")
        parser.add_argument("--title", required=True, help="Notification title")
        parser.add_argument("--message", required=True, help="Notification message content")
        parser.add_argument(
            "--notification-type",
        ...
        parser.add_argument("--send-email", action="store_true", help="Also send email notifications")
        parser.add_argument("--from-file", help="Load notification data from CSV file")
        parser.add_argument(
            "--batch-size", type=int, default=100, help="Number of notifications to process in each batch"
        )
        ...
        parser.add_argument("--schedule", help="Schedule sending for later (format: YYYY-MM-DD HH:MM)")
        parser.add_argument(
            "--process-queue", action="store_true", help="Process pending notification queue instead of creating new"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS("Bulk notification task completed successfully!"))
    ...
    def send_bulk_notification(self, options):
        ...
        if not recipients:
            self.stdout.write(self.style.WARNING("No recipients found for the specified criteria"))
            return
    ...
    def send_email_notifications(self, options, recipients):
        ...
        for recipient in recipients:
            if recipient.email:
                ...
                email_messages.append((subject, message, settings.DEFAULT_FROM_EMAIL, [recipient.email]))
        ...
        for i in range(0, len(email_messages), self.batch_size):
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Error sending email batch: {str(e)}"))
        ...
        with open(file_path, encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ...
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f"User not found: {row['username']}"))
    ...
    def schedule_notifications(self, options, recipients):
        ...
        # Store scheduled notification data
        {
            ...
            "recipients_criteria": {"type": options["recipients"], "group_name": options.get("group_name")},
        }
    ...
    def process_notification_queue(self):
        ...
        pending_notifications = Notification.objects.filter(
            email_sent=False, notification_type__in=["urgent", "important"]
        ...
        for notification in pending_notifications.iterator():
            try:
                ...
                error_count += 1
                self.stdout.write(self.style.ERROR(f"Error processing notification {notification.id}: {str(e)}"))

        self.stdout.write(
            self.style.SUCCESS(f"Queue processing completed:\n  Processed: {processed_count}\n  Errors: {error_count}")
        )
    ...
    def show_statistics(self):
        ...
        recent_notifications = Notification.objects.filter(created_at__gte=timezone.now() - timedelta(days=7)).count()
    ...
    def cleanup_old_notifications(self, days=90):
        ...
        old_notifications = Notification.objects.filter(is_read=True, created_at__lt=cutoff_date)
        ...
        self.stdout.write(self.style.SUCCESS(f"Cleaned up {count} old notifications"))
