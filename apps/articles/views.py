...
class ArticleSidebarMixin:
    ...
    def get_context_data(self, **kwargs):
        ...
        context['featured_articles'] = ArticleCacheManager.get_cached_featured_articles()[:3] if self.show_featured else []
        return context
...
class ArticleViewSet(viewsets.ModelViewSet):
    ...
    def bookmark(self, request, pk=None):
        ...
        article = self.get_object()
        if not article.is_published:
            return Response({'detail': 'Only published articles can be bookmarked.'}, status=status.HTTP_400_BAD_REQUEST)
        bookmarked = Bookmark.toggle(request.user, article)
...
class CategoryDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({'articles': page_obj.object_list, 'page_obj': page_obj, 'is_paginated': page_obj.has_other_pages()})
        return context
