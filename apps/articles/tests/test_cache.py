...
from apps.articles.cache import ArticleCacheManager, CategoryCacheManager, TagCacheManager
...
class CategoryCacheManagerTest(TestCase):
    def setUp(self):
        cache.clear()
        self.category1 = Category.objects.create(name="Technology", description="Tech articles", is_active=True)
        self.category2 = Category.objects.create(name="Science", description="Science articles", is_active=True)
        Category.objects.create(name="Inactive", description="Inactive category", is_active=False)
...
class TagCacheManagerTest(TestCase):
    def setUp(self):
        ...
        article1 = Article.objects.create(
            title="Django Article", content="Django content." * 20, author=self.user, status="published"
        )
        article1.tags.add(self.tag1)

        article2 = Article.objects.create(
            title="Python Article", content="Python content." * 20, author=self.user, status="published"
        )
    ...
    def test_popular_tags_ordering(self):
        article3 = Article.objects.create(
            title="Another Python Article", content="More Python content." * 20, author=self.user, status="published"
        )
...
class CacheIntegrationTest(TestCase):
    ...
    def test_cache_invalidation_on_article_save(self):
        article = Article.objects.create(
            title="Cache Test Article", content="Cache test content." * 20, author=self.user, status="published"
        )
