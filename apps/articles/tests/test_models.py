...
class CategoryModelTest(TestCase):
    def setUp(self):
        self.category = Category.objects.create(name="Technology", description="Tech articles")
    ...
    def test_category_absolute_url(self):
        self.assertEqual(self.category.get_absolute_url(), f"/articles/category/{self.category.slug}/")
    ...
    def test_article_count_property(self):
        ...
        Article.objects.create(
            title="Test Article", content="Test content", author=user, category=self.category, status="published"
        )
...
class TagModelTest(TestCase):
    ...
    def test_tag_absolute_url(self):
        self.assertEqual(self.tag.get_absolute_url(), f"/articles/tag/{self.tag.slug}/")


class ArticleModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email="author@example.com", first_name="John", last_name="Doe")
        self.category = Category.objects.create(name="Programming", description="Programming tutorials")
    ...
    def test_article_absolute_url(self):
        self.assertEqual(self.article.get_absolute_url(), f"/articles/{self.article.slug}/")
    ...
    def test_get_related_articles(self):
        ...
        Article.objects.create(
            title="Django Models Guide", content="Guide to Django models", author=self.user, status="published"
        ).tags.add(self.tag)
    ...
    def test_published_at_auto_set(self):
        article = Article.objects.create(
            title="Published Date Test", content="Test content", author=self.user, status="draft"
        )
    ...
    def test_unique_slug_generation(self):
        ...
        article2 = Article.objects.create(
            title="Django Testing Guide", content="Different content", author=self.user, status="published"
        )
...
class ArticleManagerTest(TestCase):
    def setUp(self):
        ...
        self.published_article = Article.objects.create(
            title="Published Article", content="Published content", author=self.user, status="published"
        )

        # Create draft article
        self.draft_article = Article.objects.create(
            title="Draft Article", content="Draft content", author=self.user, status="draft"
        )
