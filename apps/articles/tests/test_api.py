...
class ArticleAPITest(TestCase):
    ...
    def test_create_article_unauthenticated(self):
        url = reverse("articles:article-list")
        data = {"title": "Unauthorized Article", "content": "This should not be created." * 20}
        response = self.client.post(url, data)
...
class CategoryAPITest(TestCase):
    ...
    def test_create_category_staff_user(self):
        ...
        url = reverse("articles:category-list")
        data = {"name": "Science", "description": "Science articles", "is_active": True}
        response = self.client.post(url, data)
    ...
    def test_create_category_regular_user(self):
        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.token.key)
        url = reverse("articles:category-list")
        data = {"name": "Unauthorized Category", "description": "Should not be created"}
        response = self.client.post(url, data)
...
class TagAPITest(TestCase):
    ...
    def test_tag_articles_action(self):
        article = Article.objects.create(
            title="Django Tutorial", content="Django tutorial content." * 20, author=self.user, status="published"
        )
        article.tags.add(self.tag)
