...
def make_user(email, **extra):
    return User.objects.create_user(
        email=email, username=email.split("@")[0], first_name="Test", last_name="User", **extra
    )
...
class BookmarkViewTest(EngagementBase):
    ...
    def test_bookmarks_page_lists_bookmarked_articles(self):
        other = Article.objects.create(
            title="Not Bookmarked", content=LONG_CONTENT, author=self.author, status="published"
        )
        Bookmark.objects.create(user=self.reader, article=self.article)
        self.client.force_login(self.reader)
...
class RelatedArticlesTest(EngagementBase):
    ...
    def _article(self, title, tags=(), category=None, status="published"):
        article = Article.objects.create(
            title=title, content=LONG_CONTENT, author=self.author, status=status, category=category
        )
...
class EngagementAPITest(EngagementBase):
    ...
    def test_list_query_count_is_constant(self):
        ...
        for i in range(8):
            article = Article.objects.create(
                title=f"Bulk {i}", content=LONG_CONTENT, author=self.author, status="published"
            )
...
class ListingPagesTest(EngagementBase):
    ...
    def test_detail_page_renders_tags_category_and_related(self):
        ...
        related = Article.objects.create(
            title="Related Piece", content=LONG_CONTENT, author=self.author, status="published"
        )
