...
class ArticleSearchManager:
    ...
    def postgresql_search(cls, query, filters=None, use_cache=True):
        ...
        queryset = (
            Article.published.annotate(search=search_vector, rank=SearchRank(search_vector, search_query))
    ...
    def get_search_suggestions(cls, query, limit=5):
        ...
        suggestions = Article.published.filter(title__icontains=query).values_list("title", flat=True)[:limit]
    ...
    def search_autocomplete(cls, query, limit=10):
        ...
        # Article suggestions
        articles = Article.published.filter(Q(title__icontains=query) | Q(summary__icontains=query)).values(
            "id", "title", "slug"
        )[: limit // 2]

        # Category suggestions
        categories = Category.objects.filter(name__icontains=query).values("id", "name", "slug")[: limit // 4]

        # Tag suggestions
        tags = Tag.objects.filter(name__icontains=query).values("id", "name", "slug")[: limit // 4]
...
class CategorySearchManager:
    ...
    def search_categories(cls, query):
        ...
        return Category.objects.active().filter(Q(name__icontains=query) | Q(description__icontains=query))
