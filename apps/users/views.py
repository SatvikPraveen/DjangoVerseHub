...
from .serializers import (
    ...
    UserUpdateSerializer, PublicProfileSerializer,
)
...
def _user_activity(user, limit=20):
    ...
    for article in articles:
        timeline.append({
            ...
            'description': (
                f'Published "{article.title}"' if article.status == 'published'
                else f'Drafted "{article.title}"'
            ),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(_safe_redirect_target(
                request, next_url, reverse('users:profile', kwargs={'pk': user.pk})
            ))
    else:
...
def following_view(request):
    ...
    following = (
        request.user.following.filter(is_active=True)
        .select_related('profile')
        .order_by('username')
    )
    followers = (
        request.user.followers.filter(is_active=True)
        .select_related('profile')
        .order_by('username')
    )
...
def _follow_toggle(request, pk, follow):
    ...
    return _follow_response(
        request, True, message, target,
        is_following=follow, followers_count=target.followers_count,
    )
...
class UserViewSet(mixins.ListModelMixin,
    ...
    def _paginated_users(self, queryset):
        queryset = queryset.filter(is_active=True, profile__is_public=True).select_related('profile').order_by('username')
...
class ProfileViewSet(mixins.ListModelMixin,
    ...
    def search(self, request):
        ...
        profiles = (
            Profile.objects.get_public_profiles()
