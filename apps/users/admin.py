...
class ProfileInline(admin.StackedInline):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("full_name", "bio", "avatar", "cover_image")}),
        ("Personal Details", {"fields": ("gender", "location", "website"), "classes": ("collapse",)}),
        ("Social Links", {"fields": ("twitter", "linkedin", "github"), "classes": ("collapse",)}),
        ("Preferences", {"fields": ("theme", "timezone", "language"), "classes": ("collapse",)}),
        ("Privacy Settings", {"fields": ("is_public", "show_email", "show_real_name"), "classes": ("collapse",)}),
        (
            "Notifications",
            {"fields": ("email_notifications", "push_notifications", "marketing_emails"), "classes": ("collapse",)},
        ),
    )
...
class CustomUserAdmin(BaseUserAdmin):
    ...
    list_filter = ["is_active", "is_staff", "is_superuser", "email_verified", "date_joined", "last_login"]

    search_fields = ["email", "username", "first_name", "last_name"]

    readonly_fields = ["date_joined", "last_login", "login_count", "last_login_ip", "id"]
    ...
    fieldsets = (
        ...
        ("Additional Info", {"fields": ("email_verified", "login_count", "last_login_ip"), "classes": ("collapse",)}),
    )
    ...
    def avatar_preview(self, obj):
        if hasattr(obj, "profile") and obj.profile.avatar:
            return format_html(
                '<img src="{}" width="30" height="30" style="border-radius: 50%;" />', obj.profile.avatar.url
            )
        return "-"
...
class ProfileAdmin(admin.ModelAdmin):
    ...
    list_display = ["user_link", "display_name", "location", "is_public", "avatar_preview", "created_at", "updated_at"]

    list_filter = ["is_public", "gender", "theme", "email_notifications", "created_at", "updated_at"]

    search_fields = ["user__username", "user__email", "full_name", "bio", "location"]
    ...
    def avatar_preview(self, obj):
        if obj.avatar:
            return format_html('<img src="{}" width="60" height="60" style="border-radius: 8px;" />', obj.avatar.url)
        return "No avatar"
    ...
    def cover_preview(self, obj):
        if obj.cover_image:
            return format_html(
                '<img src="{}" width="120" height="40" style="border-radius: 4px;" />', obj.cover_image.url
            )
