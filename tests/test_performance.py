...
class DatabasePerformanceTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email="perf@example.com", password="testpass123", username="perfuser")
        self.client = Client()
    ...
    def test_notification_list_query_efficiency(self):
        ...
        self.assertLessEqual(query_count, 10, f"Notification list issued {query_count} queries (possible N+1)")
...
class ResponseTimeTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email="time@example.com", password="testpass123", username="timeuser")
        self.client = Client()
        self.client.force_login(self.user)
